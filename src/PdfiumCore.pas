...
type
  ...
  public
    ...
    function ReadAllTextPage: string;
...
function TPdfPage.ReadAllTextPage: string;
var
  Len: Integer;
  lCount: Integer;
begin
  ...
  begin
    ...
    Len := FPDFText_GetText(FTextHandle, 0, lCount, PWideChar(Result)) -1; // returned length includes the #0
    if Len <= 0 then
      Result := ''
    else if Len < lCount then
      SetLength(Result, Len);
  end
  else
    Result := '';
